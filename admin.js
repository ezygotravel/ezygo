...
let mediaDraft={
  ...
  gallery:{cover:'',images:[],newCover:null,newImages:[]}
};
...
function mediaIds(kind){return kind==='visa'?{cover:'vCoverPreview',images:'vDetailsPreview',coverState:'vCoverName',imagesState:'vDetailsName'}:kind==='package'?{cover:'pCoverPreview',images:'pImagesPreview',coverState:'pCoverName',imagesState:'pImagesName'}:{cover:'gCoverPreview',images:'gImagesPreview',coverState:'gCoverName',imagesState:'gImagesName'}}
...
async function cleanupRemovedMedia(paths){const list=[...new Set(paths.filter(removableStoragePath))];if(!list.length)return;try{await call('delete_media',{paths:list})}catch(err){console.warn('Media cleanup skipped:',err)}}
...
function renderGallery(){const q=($('#galleryAdminSearch')?.value||'').trim().toLowerCase();const list=data.galleries.filter(g=>!q||[g.title,g.description].join(' ').toLowerCase().includes(q));$('#galleryList').innerHTML=list.map((g,i)=>`<div class="item"><img loading="lazy" decoding="async" src="${g.cover_path?mediaUrl(g.cover_path):''}"><div><h3>${esc(g.title)}</h3><p>${esc(g.description||'')}</p></div><div class="mini"><button class="btn ghost" data-aup="${g.id}" ${i===0?'disabled':''}>↑</button><button class="btn ghost" data-adown="${g.id}" ${i===list.length-1?'disabled':''}>↓</button><button class="btn ghost" data-aedit="${g.id}">Edit</button><button class="btn danger" data-adel="${g.id}">Delete</button></div></div>`).join('')||'<div class="muted">No matching gallery items.</div>'}
...
function clearGallery(){edit.gallery=null;$('#galleryFormTitle').textContent='Add gallery';['#gTitle','#gDesc'].forEach(sel=>$(sel).value='');$('#gCover').value='';$('#gImages').value='';resetFileStates();resetDraft('gallery')}
async function saveVisa(){const btn=$('#saveVisa'),country=$('#vCountry').value.trim();if(!country)return alert('Add country.');if(!$('#vGroup').value)return alert('Create a visa section first.');setButtonBusy(btn,true);try{const old=data.visa_cards.find(x=>x.id===edit.visa),d=mediaDraft.visa;let cover=d.cover||'',details=[...d.images];if(d.newCover)cover=await upload(d.newCover.file,`visas/${slug(country)}`);if(d.newImages.length)details.push(...await uploadMany(d.newImages.map(x=>x.file),`visas/${slug(country)}/details`));const removed=[];if(old?.cover_path&&old.cover_path!==cover)removed.push(old.cover_path);for(const path of old?.detail_paths||[])if(!details.includes(path))removed.push(path);await call('upsert',{table:'visa_cards',record:{id:edit.visa||slug(country)+'-'+Date.now(),country,flag:$('#vFlag').value.trim(),group_name:$('#vGroup').value,type:$('#vType').value.trim()||'Visa',validity:$('#vValidity').value.trim()||'Check details',fee:$('#vFee').value.trim()||'Contact us',date_label:$('#vDateLabel').value.trim()||'Visa assistance',deadline:$('#vDeadline').value.trim()||'Contact us for current processing details',description:$('#vDesc').value.trim(),documents:lines('#vDocs'),cover_path:cover,detail_paths:details,active:true}});await cleanupRemovedMedia(removed);clearVisa();await refresh()}catch(e){alert(e.message||'Could not save visa.')}finally{setButtonBusy(btn,false,'Save visa')}}
async function savePackage(){const btn=$('#savePackage'),title=$('#pTitle').value.trim();if(!title)return alert('Add package title.');setButtonBusy(btn,true);try{const old=data.packages.find(x=>x.id===edit.package),d=mediaDraft.package;let cover=d.cover||'',imgs=[...d.images];if(d.newCover)cover=await upload(d.newCover.file,`packages/${slug(title)}`);if(d.newImages.length)imgs.push(...await uploadMany(d.newImages.map(x=>x.file),`packages/${slug(title)}/gallery`));const removed=[];if(old?.cover_path&&old.cover_path!==cover)removed.push(old.cover_path);for(const path of old?.image_paths||[])if(!imgs.includes(path))removed.push(path);const itinerary=$('#pItinerary').value.split('\n').map((x,i)=>{const [title2,text,meal]=x.split('|').map(a=>a.trim());return x.trim()?{day:i+1,title:title2,text,meal}:null}).filter(Boolean);await call('upsert',{table:'packages',record:{id:edit.package||slug(title)+'-'+Date.now(),title,tag:$('#pTag').value.trim()||'Travel package',destination:$('#pDestination').value.trim(),duration:$('#pDuration').value.trim(),price:$('#pPrice').value.trim()||'Ask for price',summary:$('#pSummary').value.trim(),highlights:lines('#pHighlights'),itinerary,inclusions:lines('#pInclusions'),exclusions:lines('#pExclusions'),terms:lines('#pTerms'),cover_path:cover,image_paths:imgs,active:true}});await cleanupRemovedMedia(removed);clearPackage();await refresh()}catch(e){alert(e.message||'Could not save package.')}finally{setButtonBusy(btn,false,'Save package')}}
async function saveGallery(){const btn=$('#saveGallery'),title=$('#gTitle').value.trim();if(!title)return alert('Add gallery title.');setButtonBusy(btn,true);try{const old=data.galleries.find(x=>x.id===edit.gallery),d=mediaDraft.gallery;let cover=d.cover||'',imgs=[...d.images];if(d.newCover)cover=await upload(d.newCover.file,`galleries/${slug(title)}`);if(d.newImages.length)imgs.push(...await uploadMany(d.newImages.map(x=>x.file),`galleries/${slug(title)}/images`));const removed=[];if(old?.cover_path&&old.cover_path!==cover)removed.push(old.cover_path);for(const path of old?.image_paths||[])if(!imgs.includes(path))removed.push(path);await call('upsert',{table:'galleries',record:{id:edit.gallery||slug(title)+'-'+Date.now(),title,description:$('#gDesc').value.trim(),cover_path:cover,image_paths:imgs,active:true}});await cleanupRemovedMedia(removed);clearGallery();await refresh()}catch(e){alert(e.message||'Could not save gallery.')}finally{setButtonBusy(btn,false,'Save gallery')}}
...
[['vCover','visa'],['pCover','package'],['gCover','gallery']].forEach(([input,kind])=>{const el=$('#'+input);if(el)el.onchange=()=>{const file=el.files[0];if(file)addPendingCover(kind,file);el.value=''}});
[['vDetails','visa'],['pImages','package'],['gImages','gallery']].forEach(([input,kind])=>{const el=$('#'+input);if(el)el.onchange=()=>{if(el.files.length)addPendingImages(kind,[...el.files]);el.value=''}});
resetDraft('visa');resetDraft('package');resetDraft('gallery');
tryAutoLogin();
})();