...
function clearPackage(){edit.package=null;$('#packageFormTitle').textContent='Add package';['#pTitle','#pTag','#pDestination','#pDuration','#pSummary','#pHighlights','#pItinerary','#pInclusions','#pExclusions','#pTerms'].forEach(sel=>$(sel).value='');$('#pCover').value='';$('#pImages').value='';resetFileStates();resetDraft('package')}
...
async function savePackage(){const btn=$('#savePackage'),title=$('#pTitle').value.trim();if(!title)return alert('Add package title.');setButtonBusy(btn,true);try{const old=data.packages.find(x=>x.id===edit.package),d=mediaDraft.package;let cover=d.cover||'',imgs=[...d.images];if(d.newCover)cover=await upload(d.newCover.file,`packages/${slug(title)}`);if(d.newImages.length)imgs.push(...await uploadMany(d.newImages.map(x=>x.file),`packages/${slug(title)}/images`));const removed=[];if(old?.cover_path&&old.cover_path!==cover)removed.push(old.cover_path);for(const path of old?.image_paths||[])if(!imgs.includes(path))removed.push(path);const itinerary=$('#pItinerary').value.split('\n').map((x,i)=>{const [title2,text,meal]=x.split('|').map(a=>a.trim());return x.trim()?{day:i+1,title:title2,text,meal}:null}).filter(Boolean);await call('upsert',{table:'packages',record:{id:edit.package||slug(title)+'-'+Date.now(),title,tag:$('#pTag').value.trim()||'Travel package',destination:$('#pDestination').value.trim(),duration:$('#pDuration').value.trim(),price:'Ask for price',summary:$('#pSummary').value.trim(),highlights:lines('#pHighlights'),itinerary,inclusions:lines('#pInclusions'),exclusions:lines('#pExclusions'),terms:lines('#pTerms'),cover_path:cover,image_paths:imgs,active:true}});try{await cleanupRemovedMedia(removed)}catch(cleanErr){throw new Error(`Package was saved, but removed Supabase image cleanup did not finish. ${cleanErr.message}`)}clearPackage();await refresh()}catch(e){alert(e.message||'Could not save package.')}finally{setButtonBusy(btn,false,'Save package')}}
