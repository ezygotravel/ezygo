...
function renderVisas(){$('#visaList').innerHTML=data.visa_cards.map((v,i)=>`<div class="item"><img src="${v.cover_path?mediaUrl(v.cover_path):''}"><div><h3>${esc(v.country)}</h3><p>${esc(v.group_name)} · ${esc(v.type)}</p><p>${esc(v.validity)} · ${esc(v.fee)}</p></div><div class="mini"><button class="btn ghost" data-vup="${v.id}" ${i===0?'disabled':''}>↑</button><button class="btn ghost" data-vdown="${v.id}" ${i===data.visa_cards.length-1?'disabled':''}>↓</button><button class="btn ghost" data-vedit="${v.id}">Edit</button><button class="btn danger" data-vdel="${v.id}">Delete</button></div></div>`).join('')}
